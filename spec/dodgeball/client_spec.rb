    ...
    describe Client do
      ...
      describe '#checkpoint' do
        it 'errors without an event' do
          expect { client.checkpoi(nil, SOURCE_ID, VERIFICATION_ID) }.to raise_error(ArgumentError)
        end

        it 'errors without source id' do
          expect { client.checkpoi(VERIFY_EVENT_DATA, nil, VERIFICATION_ID) }.to raise_error(ArgumentError)
        end

        it 'does not error with only the required options' do
          expect do
            client.checkpoint(VERIFY_EVENT_DATA, SOURCE_ID)
          end.to_not raise_error
        end

        it 'does not error when given string keys' do
          expect { client.checkpoint(Utils.stringify_keys(VERIFY_EVENT_DATA), SOURCE_ID) }.to_not raise_error
        end
      end
